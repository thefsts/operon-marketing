      ...
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-5 lg:flex" aria-label="Primary navigation">
          ...
          <div ref={menuRef} className="relative" onMouseEnter={() => setSolutionsOpen(true)} onMouseLeave={() => setSolutionsOpen(false)}>
            ...
            {solutionsOpen && (
              <div className="absolute left-1/2 top-[calc(100%-0.5rem)] w-[48rem] -translate-x-1/2 pt-3" role="menu">
                <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl">
                  <div className="grid grid-cols-2 gap-1">
                    {solutions.map((solution) => (
                      <Link key={solution.path} to={solution.path} onClick={closeAll} className="rounded-xl px-4 py-3 text-center transition hover:bg-slate-50" role="menuitem">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <Link to="/social-publisher-pro" onClick={closeAll} className="rounded-2xl border border-violet-100 bg-violet-50 px-5 py-4 text-center" role="menuitem">
                    ...
                    <Link to="/geofence" onClick={closeAll} className="rounded-2xl border border-cyan-100 bg-cyan-50 px-5 py-4 text-center" role="menuitem">
      ...
      {mobileOpen && (
        <div className="max-h-[calc(100vh-5rem)] overflow-y-auto border-t border-slate-200 bg-white lg:hidden">
          <nav className="mx-auto max-w-7xl px-4 py-5 text-center sm:px-6" aria-label="Mobile navigation">
            <div className="grid gap-1 sm:grid-cols-2"><Link to="/" onClick={closeAll} className="rounded-lg px-3 py-2 font-semibold">Home</Link><Link to="/platform" onClick={closeAll} className="rounded-lg px-3 py-2 font-semibold">About</Link></div>
            <div className="mt-5 py-2 text-xs font-bold uppercase tracking-[.16em] text-slate-400">Solutions</div>
            <div className="grid gap-1 sm:grid-cols-2">{solutions.map((solution) => <Link key={solution.path} to={solution.path} onClick={closeAll} className="rounded-lg px-3 py-2 text-sm font-medium text-slate-700">{solution.name}</Link>)}</div>
            <div className="mt-4 grid gap-2 sm:grid-cols-2"><Link to="/social-publisher-pro" onClick={closeAll} className="rounded-lg bg-violet-50 px-3 py-3 text-sm font-semibold text-violet-800">Social Publisher Pro™</Link><Link to="/geofence" onClick={closeAll} className="rounded-lg bg-cyan-50 px-3 py-3 text-sm font-semibold text-cyan-800">Operon Geofence™</Link></div>
            <div className="mt-4 grid gap-1 sm:grid-cols-3"><Link to="/pricing" onClick={closeAll} className="rounded-lg px-3 py-2 font-semibold">Pricing</Link><Link to="/integrations" onClick={closeAll} className="rounded-lg px-3 py-2 font-semibold">Integrations</Link><Link to="/contact" onClick={closeAll} className="rounded-lg px-3 py-2 font-semibold">Contact</Link></div>
            ...
            <Link to="/start" onClick={closeAll} className="mx-auto mt-3 block max-w-xs rounded-xl bg-slate-950 px-4 py-3 text-center font-semibold text-white">Get Started</Link>
          </nav>
        </div>
      )}
    </header>
