        ...
        <nav className="hidden items-center gap-5 lg:flex" aria-label="Primary navigation">
          ...
          >
            <button
              ...
              className="inline-flex items-center gap-1 py-7 text-sm font-semibold text-slate-700 transition hover:text-cyan-700"
              aria-haspopup="menu"
            ...
            {solutionsOpen && (
              <div className="absolute left-1/2 top-[calc(100%-0.5rem)] w-[40rem] -translate-x-1/2 pt-3" role="menu">
                <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
                  <div className="grid grid-cols-2 gap-1">
                    {solutions.map((solution) => (
                      <Link key={solution.path} to={solution.path} onClick={closeSolutions} className="rounded-xl px-4 py-3 transition hover:bg-slate-50" role="menuitem">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    <Link to="/social-publisher-pro" onClick={closeSolutions} className="rounded-xl border border-violet-100 bg-violet-50 px-4 py-3" role="menuitem">
                      <div className="font-semibold text-violet-950">Social Publisher Pro™</div>
                      <div className="mt-1 text-xs text-violet-700">Phase 1 publishing + agency CRM workflows</div>
                    </Link>
                    <Link to="/geofence" onClick={closeSolutions} className="rounded-xl border border-cyan-100 bg-cyan-50 px-4 py-3" role="menuitem">
                      <div className="flex items-center gap-2 font-semibold text-cyan-950"><MapPin className="h-4 w-4" />Operon Geofence™</div>
                      <div className="mt-1 text-xs text-cyan-700">Location intelligence for workforce, operations & marketing</div>
                    </Link>
                  </div>
                </div>
              </div>
      ...
      {mobileOpen && (
        <div className="border-t border-slate-200 bg-white lg:hidden">
          <nav className="mx-auto max-w-7xl space-y-1 px-4 py-5 sm:px-6" aria-label="Mobile navigation">
            <Link to="/" onClick={() => setMobileOpen(false)} className="block rounded-lg px-3 py-2 font-semibold">Home</Link>
            <Link to="/platform" onClick={() => setMobileOpen(false)} className="block rounded-lg px-3 py-2 font-semibold">About</Link>
            <div className="py-2 text-xs font-bold uppercase tracking-[.16em] text-slate-400">Solutions</div>
            <div className="grid gap-1 sm:grid-cols-2">{solutions.map((solution) => <Link key={solution.path} to={solution.path} onClick={() => setMobileOpen(false)} className="rounded-lg px-3 py-2 text-sm font-medium text-slate-700">{solution.name}</Link>)}</div>
            <Link to="/social-publisher-pro" onClick={() => setMobileOpen(false)} className="mt-2 block rounded-lg bg-violet-50 px-3 py-3 text-sm font-semibold text-violet-800">Social Publisher Pro™</Link>
            <Link to="/geofence" onClick={() => setMobileOpen(false)} className="block rounded-lg bg-cyan-50 px-3 py-3 text-sm font-semibold text-cyan-800">Operon Geofence™</Link>
            <Link to="/pricing" onClick={() => setMobileOpen(false)} className="block rounded-lg px-3 py-2 font-semibold">Pricing</Link>
            <Link to="/integrations" onClick={() => setMobileOpen(false)} className="block rounded-lg px-3 py-2 font-semibold">Integrations</Link>
            <Link to="/contact" onClick={() => setMobileOpen(false)} className="block rounded-lg px-3 py-2 font-semibold">Contact</Link>
            <div className="px-3 py-2"><LanguageSwitcher /></div>
            <a href={`${APP_URL}/login`} className="block rounded-lg px-3 py-2 font-semibold">Login</a>
